...
fn test_we_use_the_count_properly() {
    let rrule_result =
        convert_to_rrule("FREQ=MONTHLY;COUNT=27;INTERVAL=1;BYHOUR=9;BYMINUTE=1;BYMONTHDAY=28,27")
            .unwrap();
...
fn test_fortnightly_rules_2() {
    let rrule_result = convert_to_rrule(
        "FREQ=WEEKLY;INTERVAL=2;BYHOUR=17;BYMINUTE=0;TZID=Australia/Sydney;DTSTART=2019062T000003",
    )
    .unwrap();
